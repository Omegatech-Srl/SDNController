...
# Version               | 6.0.4
# Application version   | 8.0.7-7a3d06144a
# Debian Repo version   | 8.0.7-24256-1
# Author                | Matteo Giustini
# Email                 | iris180196@gmail.com
...
$$ |  $$ |$$ |  $$ |$$ |$$ |      $$ |      $$ |  $$\ $$ |  $$ |$$ |  $$ | $$ |$$\ $$ |      $$ |  $$ |$$ |$$ |$$   ____|$$ |      
\$$$$$$  |$$ |  $$ |$$ |$$ |      $$ |      \$$$$$$  |\$$$$$$  |$$ |  $$ | \$$$$  |$$ |      \$$$$$$  |$$ |$$ |\$$$$$$$\ $$ |      
 \______/ \__|  \__|\__|\__|      \__|       \______/  \______/ \__|  \__|  \____/ \__|       \______/ \__|\__| \_______|\__|      
...
$$$\    $$$ |           $$ |    $$ |                              $$  __$$\ \__|                      $$ |    \__|          \__|   
$$$$\  $$$$ | $$$$$$\ $$$$$$\ $$$$$$\    $$$$$$\   $$$$$$\        $$ /  \__|$$\ $$\   $$\  $$$$$$$\ $$$$$$\   $$\ $$$$$$$\  $$\    
$$\$$\$$ $$ | \____$$\\_$$  _|\_$$  _|  $$  __$$\ $$  __$$\       $$ |$$$$\ $$ |$$ |  $$ |$$  _____|\_$$  _|  $$ |$$  __$$\ $$ |   
$$ \$$$  $$ | $$$$$$$ | $$ |    $$ |    $$$$$$$$ |$$ /  $$ |      $$ |\_$$ |$$ |$$ |  $$ |\$$$$$$\    $$ |    $$ |$$ |  $$ |$$ |   
$$ |\$  /$$ |$$  __$$ | $$ |$$\ $$ |$$\ $$   ____|$$ |  $$ |      $$ |  $$ |$$ |$$ |  $$ | \____$$\   $$ |$$\ $$ |$$ |  $$ |$$ |   
$$ | \_/ $$ |\$$$$$$$ | \$$$$  |\$$$$  |\$$$$$$$\ \$$$$$$  |      \$$$$$$  |$$ |\$$$$$$  |$$$$$$$  |  \$$$$  |$$ |$$ |  $$ |$$ |   
\__|     \__| \_______|  \____/  \____/  \_______| \______/        \______/ \__| \______/ \_______/    \____/ \__|\__|  \__|\__|   
...
add_mongodb_repo() {
  ...
  if [[ "${mongodb_org_v::2}" == '42' ]] || [[ "${add_mongodb_42_repo}" == 'true' ]]; then
    ...
    if [[ "${try_different_mongodb_repo}" == 'true' ]] || [[ "${architecture}" != "amd64" ]]; then
      ...
      if [[ "${os_codename}" =~ (stretch|continuum) ]]; then
        ...
        mongodb_codename="ubuntu xenial"
        mongodb_repo_type="multiverse"
      elif [[ "${os_codename}" =~ (bionic|tara|tessa|tina|tricia|hera|juno|focal|groovy|hirsute|impish|jammy|kinetic|lunar|mantic) ]]; then
        mongodb_codename="ubuntu bionic"
        mongodb_repo_type="multiverse"
      else
        mongodb_codename="ubuntu xenial"
  ...
  if [[ -n "${mongodb_version_major_minor}" ]]; then
    if ! [[ -e "/usr/share/keyrings/mongodb-server-${mongodb_version_major_minor}.gpg" ]]; then
      ...
      aptkey_depreciated
      if [[ "${apt_key_deprecated}" == 'true' ]]; then
        if wget -qO - "${repo_http_https}://www.mongodb.org/static/pgp/server-${mongodb_version_major_minor}.asc" | gpg --dearmor | tee -a "/usr/share/keyrings/mongodb-server-${mongodb_version_major_minor}.gpg" &> /dev/null; then echo -e "${GREEN}#${RESET} Successfully added the key for MongoDB ${mongodb_version_major_minor}! \\n"; signed_by_value=" signed-by=/usr/share/keyrings/mongodb-server-${mongodb_version_major_minor}.gpg"; else echo -e "${RED}#${RESET} Failed to add the key for MongoDB ${mongodb_version_major_minor}...\\n"; abort; fi
      else
        if wget -qO - "${repo_http_https}://www.mongodb.org/static/pgp/server-${mongodb_version_major_minor}.asc" | apt-key add - &> /dev/null; then echo -e "${GREEN}#${RESET} Successfully added the key for MongoDB ${mongodb_version_major_minor}! \\n"; else echo -e "${RED}#${RESET} Failed to add the key for MongoDB ${mongodb_version_major_minor}...\\n"; abort; fi
      fi
    else
      signed_by_value=" signed-by=/usr/share/keyrings/mongodb-server-${mongodb_version_major_minor}.gpg"
    fi
    ...
    if echo "deb [ ${arch}${signed_by_value} ] ${repo_http_https}://repo.mongodb.org/apt/${mongodb_codename}/mongodb-org/${mongodb_version_major_minor} ${mongodb_repo_type}" &> "/etc/apt/sources.list.d/mongodb-org-${mongodb_version_major_minor}.list"; then
...
cleanup_archived_repos() {
  if [[ "${archived_repo}" == "true" ]]; then
    repo_file_patterns=( "deb.debian.org\\/debian ${os_codename}" "deb.debian.org\\/debian\\/ ${os_codename}" "ftp.*.debian.org\\/debian ${os_codename}" "ftp.*.debian.org\\/debian ${os_codename}" "ftp.*.debian.org\\/debian\\/ ${os_codename}" "security.debian.org ${os_codename}" "security.debian.org\\/ ${os_codename}" "ftp.debian.org\\/debian ${os_codename}" "ftp.debian.org\\/debian\\/ ${os_codename}" "http.debian.net\\/debian ${os_codename}" "http.debian.net\\/debian\\/ ${os_codename}" "*.archive.ubuntu.com\\/ubuntu ${os_codename}" "*.archive.ubuntu.com\\/ubuntu\\/ ${os_codename}" "archive.ubuntu.com\\/ubuntu ${os_codename}" "archive.ubuntu.com\\/ubuntu\\/ ${os_codename}" "security.ubuntu.com\\/ubuntu ${os_codename}" "security.ubuntu.com\\/ubuntu\\/ ${os_codename}" )
    ...
    done < <(find /etc/apt/ -type f -name "*.list" -exec grep -ilE 'debian.org|debian.net|ubuntu.com' {} +)
  fi
}
cleanup_archived_repos
  ...
  if [[ $(find /etc/apt/ -name "*.list" -type f -print0 | xargs -0 cat | grep -c "^deb ${add_repositories_http_or_https}://$(echo "${repo_url}" | sed -e 's/https\:\/\///g' -e 's/http\:\/\///g')${repo_url_arguments} ${repo_codename}${repo_arguments}") -eq 0 ]]; then
    if [[ "${apt_key_deprecated}" == 'true' ]]; then
      if [[ -n "${repo_key}" && -n "${repo_key_name}" ]]; then
        if gpg --no-default-keyring --keyring "/usr/share/keyrings/${repo_key_name}.gpg" --keyserver hkp://keyserver.ubuntu.com:80 --recv-keys "${repo_key}" &> /dev/null; then
          signed_by_value_repo_key="[ /usr/share/keyrings/${repo_key_name}.gpg ] "
        else
...
if ! dpkg -l jq 2> /dev/null | awk '{print $1}' | grep -iq "^ii\\|^hi"; then
  ...
  if ! DEBIAN_FRONTEND='noninteractive' apt-get -y "${apt_options[@]}" -o Dpkg::Options::='--force-confdef' -o Dpkg::Options::='--force-confold' install jq &>> "${eus_dir}/logs/required.log"; then
    ...
    elif [[ "${repo_codename}" =~ (jammy|kinetic|lunar|mantic) ]]; then
      repo_arguments=" main"
...
mongo_version_supported_short="36"
...
if [[ "${first_digit_unifi}" -le '5' && "${second_digit_unifi}" -le '13' ]]; then
  ...
  mongo_version_supported_short="34"
fi
...
if [[ "${first_digit_unifi}" -gt '7' ]] || [[ "${first_digit_unifi}" == '7' && "${second_digit_unifi}" -ge "5" ]]; then
  ...
  mongo_version_supported_short="44"
...
if [[ "${mongodb_version_installed_no_dots::2}" -gt "${mongo_version_supported_short}" ]]; then
...
run_apt_get_update
apt_mongodb_check
if [[ "${MONGODB_ORG_CACHE::2}" -gt "${mongo_version_supported_short}" ]]; then
...
if [[ "${MONGODB_CACHE::2}" -gt "${mongo_version_supported_short}" || "${MONGO_TOOLS_CACHE::2}" -gt "${mongo_version_supported_short}" ]]; then
...
libssl_installation() {
  if [[ "${libssl_install_required}" == 'true' ]]; then
    ...
    if [[ "${libssl_install_success}" != 'true' ]]; then echo -e "${RED}#${RESET} Failed to install libssl...\\n"; abort; fi
...
mongodb_installation_armhf() {
  ...
  if [[ "${os_codename}" =~ (stretch) ]]; then
    os_codename="buster"
    raspbian_code_name_changed="true"
  fi
  if [[ -f "/etc/apt/sources.list.d/glennr_armhf.list" ]]; then rm --force "/etc/apt/sources.list.d/glennr_armhf.list"; fi
  echo "deb ${signed_by_value_raspbian}http://archive.raspbian.org/raspbian ${os_codename} main contrib non-free rpi" &> /etc/apt/sources.list.d/glennr-install-script.list
  hide_apt_update="true"
  run_apt_get_update
  ...
  if [[ "${raspbian_code_name_changed}" == 'true' ]]; then get_distro; fi
  sleep 3
}
  ...
  if [[ "${broken_unifi_install}" == 'true' ]]; then
    ...
    if [[ -n "${previous_mongodb_version}" ]]; then
      ...
      if [[ "${previous_mongodb_version::2}" == '26' ]]; then
        ...
        mongo_version_not_supported="4.0"
        mongo_version_supported_short="32"
        ...
        mongo_version_supported="4.4.999"
        mongo_version_not_supported="4.5"
        mongo_version_supported_short="40"
      elif [[ "${previous_mongodb_version::2}" == '42' ]]; then
        unset arm64_mongodb_version
        add_mongodb_42_repo="true"
        mongo_version_supported="4.4.999"
        mongo_version_not_supported="4.5"
        mongo_version_supported_short="42"
...
if [[ "${script_option_skip}" != 'true' || "${script_option_add_repository}" == 'true' ]] && [[ "${unifi_repo_supported}" == "true" ]]; then
  ...
  case "$yes_no" in
      [Yy]*|"")
        ...
        if wget -qO - "https://dl.ui.com/unifi/unifi-repo.gpg" | gpg --dearmor | tee -a "/usr/share/keyrings/unifi-repo.gpg" &> /dev/null; then
          ...
          if [[ "${architecture}" == 'arm64' ]]; then arch="arch=arm64"; elif [[ "${architecture}" == 'amd64' ]]; then arch="arch=amd64"; else arch="arch=amd64,arm64"; fi
          if echo "deb [ ${arch} signed-by=/usr/share/keyrings/unifi-repo.gpg ] https://www.ui.com/downloads/unifi/debian unifi-${first_digit_unifi}.${second_digit_unifi} ubiquiti" &> /etc/apt/sources.list.d/100-ubnt-unifi.list; then
